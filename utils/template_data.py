...
EXAMPLE_SUBROUNDS = [("Subround 1.1", 0), ("Subround 1.2", 0), ("Subround 2.1", 1), ("Subround 2.2", 1)]

EXAMPLE_WORDS = [(f"Word {i}", i % 6 + 1) for i in range(50)]
EXAMPLE_PLAYERS = [(f"Player {2 * i}", f"Player {2 * i + 1}", (i % 4) // 2, i % 4, i + 1) for i in range(20)]
