...
class Round:
    def __init__(self, name: str, participants: List[PlayPair], difficulty: int, word_bank: WordBank) -> None:
        self.name: str = name
        self.participants: List[PlayPair] = participants
        self.difficulty: int = difficulty
        self.word_bank = word_bank
        self.subrounds: List[Subround] = []
        self.results: CounterT[PlayPair, int] = Counter(participants)
        self.state = RoundStatus.not_started

    def add_subround(self, participants: List[PlayPair], words_to_take: int) -> bool:
        if self.state == RoundStatus.completed:
            return False
        if len(participants) < 2:
            return False
        if not set(participants).issubset(self.participants):
            return False
    ...
    def get_top_n(self, n: int) -> List[PlayPair]:
...
class Subround:
    def __init__(self, participants: List[PlayPair], words: List[str]) -> None:
    ...
    def split_players_to_games(self, amount_of_games: int) -> bool:  # TODO: replace bool with exceptions
        if self.state != SubroundStatus.not_split:
            return False
        if amount_of_games * 2 > len(self.participants):
            return False

        parts = split_into_near_equal_parts(self.participants, amount_of_games)
        for part in parts:
